...
pub struct LabelStringStore<'a, S: PageStore> {
    storage: &'a mut StorageManager<S>,
}

impl<'a, S: PageStore> LabelStringStore<'a, S> {
    pub fn new(storage: &'a mut StorageManager<S>) -> Self {
        LabelStringStore { storage }
    }
    ...
    fn insert_into_page(
        ...
        let mut page_id_val = self.storage.footer.first_label_string_page.get();
